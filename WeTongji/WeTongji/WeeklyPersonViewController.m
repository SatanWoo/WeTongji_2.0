...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        ...
        if ([self.starList count] == 2) {
            ...
            [self configureWeeklyPersonCellBg:@"list_single_cell" forCell:((WeeklyPersonCell *)cell)];
        } else if ([self.starList count] > 2) {
            if (indexPath.row == 1) {
                ((WeeklyPersonCell *)cell).partition.hidden = NO;
                [self configureWeeklyPersonCellBg:@"list_header" forCell:((WeeklyPersonCell *)cell)];
            } else if (indexPath.row == [self.starList count] - 1) {
                ((WeeklyPersonCell *)cell).partition.hidden = YES;
                [self configureWeeklyPersonCellBg:@"list_footer" forCell:((WeeklyPersonCell *)cell)];
            } else {
                ((WeeklyPersonCell *)cell).partition.hidden = NO;
                [self configureWeeklyPersonCellBg:@"list_cell" forCell:((WeeklyPersonCell *)cell)];
            }
        }
